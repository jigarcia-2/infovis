...
def main():
    input_path = selected_path / "music_categorized.csv"
    output_path = aggregated_path / "music_daily.csv"
    ...
    print("\nArchivo music_categorized leído correctamente.")
    ...
    daily = (
        ...
        .agg(
            music_minutes_total=("minutes_played", "sum"),
            music_unique_artists=("artistName", "nunique"),
            music_main_context=("music_context", lambda x: most_frequent_value(x, "sin_dato")),
        )
    )

    daily.to_csv(output_path, index=False, encoding="utf-8")

    print("\nDataset diario de music creado:")
    print(daily.head())
